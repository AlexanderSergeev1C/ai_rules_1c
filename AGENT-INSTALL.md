...
- **Source** — local clone of `1c-rules` if the user pointed at one; otherwise `https://github.com/AlexanderSergeev1C/ai_rules_1c` for this fork (upstream default: `https://github.com/comol/ai_rules_1c`).
...
**Remote MCP host (this fork).** Section 3 of `.dev.env.example` ships pre-filled defaults for the Windows + Mac mini workflow (`MCP_HOST`, `MCP_SSH_HOST_ALIAS`, sync paths, `MCP_IMAGE_TAG=arm64`). Do not re-prompt for these when copying the template. After configuration export, guide the user through `/synctomcp` and `/installmcp` — see `README.md` (*Двухмашинная установка*) and `content/rules/sync-to-mcp-host.md`. Empty `MCP_HOST` or `localhost` keeps upstream single-machine MCP on Windows.
